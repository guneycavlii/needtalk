...
class KonusmaT extends StatefulWidget {

  final Terapist currentTerapist;
  final String sohbetEdilenUsername;
  final String sohbetEdilenUserID;
  final String sohbetEdilenUserProfilURL;

  KonusmaT({this.currentTerapist, this.sohbetEdilenUsername,this.sohbetEdilenUserID,this.sohbetEdilenUserProfilURL});

  @override
  _KonusmaState createState() => _KonusmaState();
}

class _KonusmaState extends State<KonusmaT> with TickerProviderStateMixin{
//Alanı kontrol etmek amacıyla controller oluşturuyoruz.
 ...
 controller.stop();
  ...
  Konusma(controller: controller);

  controller.reverse(from: controller.value == 0.0 ? 1.0 : controller.value);
  

    Terapist _currentTerapist = widget.currentTerapist;

    // String _sohbetEdilenUserName = widget.sohbetEdilenUsername;
    String _sohbetEdilenUserID = widget.sohbetEdilenUserID;
    // String _sohbetEdilenUserProfilURL = widget.sohbetEdilenUserProfilURL;
...
//             ],),)
          ...
          Expanded(
            ...
            stream: _userModel.getMessagesT(
                _currentTerapist.terapistID,_sohbetEdilenUserID),
                      ...
                      onPressed: () async {
                        ...
                        if (_mesajController.text.trim().length > 0) {
                          ...
                          Mesaj _kaydedilecekMesaj = Mesaj(
                              kimden: _currentTerapist.terapistID,
                              kime: _sohbetEdilenUserID,
                              bendenMi: true,
                              mesaj: _mesajController.text);

                          var sonuc =
                              await _userModel.saveMessageT(_kaydedilecekMesaj);
                          if (sonuc) {
          ...
          children: <Widget>[
            Row(
              children: <Widget>[
                CircleAvatar(
                  backgroundImage:
                      NetworkImage(widget.sohbetEdilenUserProfilURL),
                ),
                Flexible(
                  child: Container(
                    decoration: BoxDecoration(
